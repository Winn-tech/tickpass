...
const EventCheckout: React.FC = () => {
  ...
  const showTimeoutModal = timeRemaining <= 0
    ...
    getTicketDetails(eventId)
      .then((response: TicketDetailsResponse) => {
        if (response.status === 'success' && response.data) {
          const tickets: TicketType[] = response.data.ticketClasses.map((ticket, index: number) => ({
            id: index + 1,
            name: ticket.name,
            price: ticket.price,
  ...
  const subtotal = useMemo(() => {
    return Object.entries(selectedTickets).reduce((sum, [id, qty]) => {
      ...
      return sum + (ticket ? ticket.price * Number(qty) : 0)
    }, 0)
