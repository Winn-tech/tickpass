...
export type FormData = Omit<CreateEventDto, 'createdBy'> & {
  imageUrl: string | File;
};
export type UpdateFormData = <K extends keyof FormData>(field: K, value: FormData[K]) => void;
...
export default function CreateEventPage() {
  ...
  const handleNext = () => {
    if (currentStep === 1 && user) {
      setFormData((prev) => ({
        ...
        organizerName:
          prev.organizerName ||
          (user.firstName ? `${user.firstName} ${user.lastName}`.trim() : user.businessName ?? ''),
        organizerEmail: prev.organizerEmail || user.email || '',
        organizerPhone: prev.organizerPhone || user.phoneNumber || '',
      }));
    }
  ...
  const uploadImageToCloudinary = async (file: File): Promise<string> => {
    return new Promise((resolve, reject) => {
      ...
      data.append('folder', 'tickets_images');
