...
const required = (field) => [true, `An event must have a ${field}`];
const max = (field, n) => [n, `An event ${field} cannot exceed ${n} characters`];
...
eventSchema.pre('validate', function (next) {
    ...
    if (dup)
        return next(new Error('Ticket-class names must be unique within the event'));
    next();
});
...
eventSchema.virtual('totalSold').get(function () {
    if (!this.ticketClasses || this.ticketClasses.length === 0)
        return 0;
...
eventSchema.statics.getClass = function (eventId, className) {
    return this.findOne({ _id: eventId, 'ticketClasses.name': className }, { 'ticketClasses.$': 1 });
};
